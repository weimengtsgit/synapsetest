...
1. 用例名称（name）：简明扼要描述测试目标，**必须100%使用中文，不得包含任何英文字母！**
2. 前置条件：执行测试前需要满足的条件
3. 测试步骤：详细的操作步骤（编号）
4. 预期结果：每个步骤的预期输出
...
【输出格式】（JSON）
🚨 最后一次警告：name字段必须全部使用中文，绝对不允许出现英文字母（a-z, A-Z）！违反者将被丢弃！
...
⚠️ 注意：上面示例中的name字段都是纯中文，没有任何英文字母！请严格遵守！
```

【注意事项】
- 🚨 **name字段必须100%纯中文，不得包含任何英文字母（包括a-z, A-Z）！**
- 🚨 **不要使用拼音或英文单词，即使是专业术语也要用中文翻译！**
- 🚨 **违反此规则的测试用例将被系统自动过滤丢弃，不会保存到数据库！**
- ✅ 确保测试用例覆盖正常流程、异常流程和边界条件
- ✅ 优先级根据业务重要性和风险程度判断
- ✅ 测试步骤要具体可执行
- ✅ 预期结果要明确可验证

现在请严格按照上述要求开始生成测试用例（记住：name字段必须纯中文！）：
"""
...
【注意】
- 🚨 **name字段必须100%纯中文，不得包含任何英文字母！违反此规则的用例将被系统自动丢弃！**
- 🚨 **示例中的name都是纯中文，请严格遵守这个格式！**
...
【注意】
🚨 **name字段必须100%纯中文，不得包含任何英文字母！违反此规则的用例将被系统自动丢弃！**
🚨 **不要使用"Test API"、"Verify POST"等英文，必须全部使用中文！**
